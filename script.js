  ...
  function handleCenterDetection() {
    ...
    if (centered !== currentCentered) {
      ...
      centerTimer = setTimeout(() => {
        ...
        // Notifica che il peso è stato selezionato
        document.dispatchEvent(new CustomEvent("weightSelected", { detail: { weight: centered.textContent } }));
      }, 500);
    }
  }
...
function createDropdown(dropdownEl, options = {}) {
  ...
  dropdownEl.disabled = true; // Disabilita il menu a tendina inizialmente
  dropdownEl.classList.add("disabled-dropdown");
  ...
  // Aggiungi un effetto visivo di errore se si tenta di interagire con il menu disabilitato
  dropdownEl.addEventListener("click", () => {
    if (dropdownEl.disabled) {
      dropdownEl.classList.add("error");
      dropdownEl.classList.add("shake");

      // Trova il carosello associato al menu a tendina
      const carouselEl = document.querySelector(".carousel");
      if (carouselEl) {
        carouselEl.classList.add("error"); // Aggiungi il bordo rosso
        carouselEl.classList.add("shake"); // Aggiungi l'effetto shake

        // Rimuovi l'effetto dopo 1 secondo
        setTimeout(() => {
          carouselEl.classList.remove("error");
          carouselEl.classList.remove("shake");
        }, 1000);
      }

      setTimeout(() => {
        dropdownEl.classList.remove("shake");
      }, 500);
    }
  });

  // Abilita il menu a tendina quando il peso è stato selezionato
  document.addEventListener("weightSelected", () => {
    dropdownEl.disabled = false;
    dropdownEl.classList.remove("disabled-dropdown");
  });
}
